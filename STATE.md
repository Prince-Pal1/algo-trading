...
**Tests:** 74 pass (`tests/test_data/test_cvd.py` 48, `tests/test_data/test_agg_trades.py` 26). The two load-bearing ones are the aggressor-convention tests and the streaming-vs-vectorized parity test across 1m/5m/15m on 500 pseudo-random trades. End-to-end CLI smoke run on a synthetic series built to diverge (price up, aggressive selling dominant) correctly flagged bearish divergences and a `sell_absorbed` bar.
