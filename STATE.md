...
**Still open from the same conversation:** volume-traded-at-price vs displayed size (the true iceberg signature) — deferred until depth is confirmed working on a live feed. And prior-test *outcomes* as level memory; `test_count` counts tests but does not remember how they resolved.

**Next:** Phase 2 — run it in shadow mode on BTC, ignore everything it says, collect outcomes.
