...
| IC Markets Feed | `src/data/feeds/icmarkets_feed.py` | `ctrader-open-api`, Twisted (threaded reactor), Types | TradingEngine (gold) | ✅ Session 23 D1 — full 5-stage auth walk; Session 24 (2026-05-05) added `ProtoOASubscribeLiveTrendbarRes` ack handler + `_send_loud()` for WARNING-level errbacks on subscribe path + 30s pending-subscribe audit via `reactor.callLater` (root-causing the 87h gold STALE outage from silent trendbar resubscribe failure on Sun 22:27 UTC reconnect). Reactor runs in worker thread, asyncio stays on the main loop. Live validated on XAUUSD 1h. |
...
| Watchdog | `scripts/watchdog.py` | stdlib only + `osascript` notify | launchd | ✅ v2 2026-05-05 — multi-engine (`heartbeat.json` + `heartbeat_gold.json`); `launchctl kickstart -k gui/<uid>/<label>` instead of broken PID-file kill (PID file was never written, kill never fired); candle-stale threshold bumped 1800→7200s (1h candles need 2× grace); macOS notification on STALE; 600s kick cooldown to prevent thrash |
