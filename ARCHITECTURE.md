...
| Fee Profile Registry | `src/backtest/fee_profiles.py` + `config/broker_fees.toml` | tomllib, costs.py | Every backtest | ✅ 2026-04-14 task #106 (research-calibrated named broker × platform × scenario profiles. 7 profiles cover IC Markets cTrader/MT4 × XAUUSD/FX × normal/news/stress + pine_zero_cost. Sources cited inline. `make_fee_model(name)`, `cost_as_pct_of_margin(...)` for leverage analysis. See `docs/BROKER_FEES.md`.) |
...
| Strategy Storage | `src/strategies/storage.py` | sqlite3, BaseStrategy (lazy), deep_backtest helpers | `run_deep_backtest` auto-hook, `scripts/strategies.py` | ✅ 2026-04-15 task #117 (G.8) — 2-table versioned registry (`strategies` + `strategy_versions`) in `data/trades.db` with WAL + FK + retry-on-busy. Auto-captures `max_return_cell` + `max_return_sane` flag + relative `report_html_path` from every `run_deep_backtest()` call. Slug scheme `{mode}_L{int(baseline)}_{tf}` + 6-char param-hash collision fallback. Do-not-clobber UPSERT preserves user-set description/family/tags. |
| Strategies CLI | `scripts/strategies.py` | `src.strategies.storage`, optional `rich` | Manual run | ✅ 2026-04-15 task #117 (G.8) — `list`/`show`/`register` subcommands; amber vanity-flag on max-return cells with < 30 trades, ≥ 60% DD, or Calmar ≤ 0.2. |
...
| `strategy_versions` | storage.record_deep_backtest_result() (auto) | storage.list_versions() / get_version() + scripts/strategies.py show | Per-variant data. One row per (strategy_id, version_slug). Fields: leverage_mode, baseline_leverage, timeframe, params_json, verdict, **max_return_pct + max_return_cell_json (absolute-max from matrix, task #117 G.8)** + max_return_sane flag + max_return_warning, best_calmar_* (existing best_cell), report_dir + report_html_path (relative to REPO_ROOT), last_backtested_at. Slug scheme: `{mode}_L{int(baseline)}_{tf}` + 6-char param-hash collision fallback. |
