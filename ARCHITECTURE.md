...
| Module | File | Depends On | Used By | Status |
|---|---|---|---|---|
| Level Registry | `src/flow/level_registry.py` | `tomllib`, `config/levels.toml` | Flow Engine, monitor | ✅ 2026-09-22 (**Zone is DIRECTIONAL**: `width` runs from the level into the side price penetrates — short 4000 w100 → 4000-4100, long 4000 w100 → 3900-4000. The level is the edge you trade off; the width is the risk. TOML levels, read AND written — `upsert`/`remove`/`set_enabled`/`save` back the live page's editor, so the file stays the single source of truth. `first_seen_ms` is persisted to the file, not just held in memory; preserves it across reloads — the only thing separating a level written before price arrived from hindsight; malformed file keeps prior levels rather than disarming live ones) |
| Zone Features | `src/flow/features.py` | Types | Evidence, Zone State | ✅ 2026-09-22 (`ZoneAccumulator` — **~1.17 µs/tick**, bounded deques, no allocation per tick. `absorption = |delta_ratio| × (1 − min(1, range_ratio))` × trade-count confidence ramp. Also tape velocity + acceleration, large-print share vs market norm, **probe tracking** for the lower-volume retest, and **iceberg detection** (`traded / max-ever-displayed` per price bucket). `MarketContext` supplies baselines measured outside the zone plus the tick size. `prior_held`/`prior_failed` are snapshotted at zone entry so the read cannot shift mid-test) |
...
| Zone State Machine | `src/flow/zone_state.py` | Features, Evidence, Types | `scripts/flow_monitor.py` | ✅ 2026-09-22 (`ZoneMonitor` IDLE→APPROACHING→EVALUATING→CONFIRMED/INVALIDATED→IDLE; invalidation beats confirmation; one signal per test. **`ZonePhase` models the attack/defence sequence** — WATCHING→ABSORBING→TURNING (or FAILING) — and a signal fires only on the TURN, not on absorption alone; `require_turn=False` is the comparison arm. **Outcome classification** feeds Level Memory: invalidation → `failed`, leaving on the favourable side → `held`, a confirmation that later breaches its own invalidation → `failed`, anything else → nothing recorded (inconclusive is not evidence); one outcome per test. `FlowEngine` routes ticks, builds `MarketContext`, infers tick size from book level spacing (latched), tracks tape lag AND publish-time staleness. `RollingExtremes` gives the baseline range in O(1) — the hot path is ~10-17 µs/tick and flat from 5 to 500 trades/sec) |
| Level Memory | `src/flow/level_memory.py` | `orjson` | Zone State, Evidence | ✅ 2026-09-22 (**optional, OFF by default** — `--level-memory`. Persists how each level resolved (held / failed) across tests and restarts, age-decayed at 14 days. `failed` weighted 2× `held`: a break proves the defender is absent, a hold may just mean nobody tested it. Deliberately in tension with the `repeated_test` rule — different things measured) |
| Flow Tape | `src/flow/tape.py` | Types | Live Server, page | ✅ 2026-09-22 (rolling columns: candle + aggressor volume + footprint (volume at price, split by side) + a book sample per column. Keyed by ABSOLUTE price-bucket index, sparse — price drifts, and a fixed grid would clip or rebuild. Row height is sized from the narrowest active ZONE, not the market range, because the chart window is zone-anchored; changing it clears the history, since every index means a different price under a new divisor) |
| Situation Narrative | `src/flow/narrative.py` | Features, Level Registry | Live page | ✅ 2026-09-22 (plain-English read: where price sits, who is pushing, whether it is working, the last 30s. States measurements only — a test asserts no advice vocabulary leaks. Server-side so it is testable and loggable; runs at UI cadence, never on the tick path) |
...
| Live Server + Page | `src/flow/live_server.py` + `live_page.html` | `websockets`, `orjson` | Browser | ✅ 2026-09-22 (serves page on `GET /`, pushes snapshots on `/ws` at 5 Hz. NOT Streamlit — see Known Gotchas. Lag is the headline number; 50-signal ring buffer replayed on connect. **Two-way**: the page sends level edits + settings back via `on_command`, so the handshake checks `Origin` — see Known Gotchas. **Canvas chart**: depth heatmap (sequential blue ramp) + zone band + delta-coloured candles + footprint rail + CVD pane + crosshair, all hand-drawn on canvas with no library and no CDN. Closed columns are rasterised once per `seq` into an offscreen canvas and blitted; only the forming column redraws at 5 Hz) |
| Flow Monitor CLI | `scripts/flow_monitor.py` | Binance WS feed, all of `src/flow/` | Manual run | ✅ 2026-09-22 (`--symbol --depth --threshold --level-memory --no-turn --port`; uvloop; levels hot-reload every 10s. Owns the UI command handler: `level.upsert` / `level.remove` / `level.enable` / `config.set`, all synchronous on the server task. Threshold, require-turn and level-memory are live-editable; `--depth` still needs a restart) |
...
binance_ws @trade ──> FlowEngine.on_tick()          HOT PATH ~0.9 µs/tick
       ...
       │                    └─> LevelMemory   how this level resolved before (optional)
...
| Date | Description |
|---|---|
| 2026-09-22 | **A dead feed looked exactly like a calm market.** `lag_ms` is measured when a tick ARRIVES, so when nothing arrives it freezes at its last good value — the page showed `40ms`, green, indefinitely, while ten minutes of nothing went by, and the WebSocket to the monitor stayed up so `link: live` agreed. On a console whose stated premise is *a stale score is worse than no score*, this was the most dangerous defect in the stack. `snapshot()` now also publishes `stale_ms`, measured at PUBLISH time against the last tick so it climbs whether or not anything arrives; the page leads with whichever is worse and shows a red NO DATA banner past 10s. Pinned by `TestStaleness::test_lag_alone_would_have_lied`. General shape: **a freshness metric computed on arrival cannot detect absence** — it has to be computed on read. |
| 2026-09-22 | **`_baseline_range` was a full window scan on every tick — 99.7% of the hot path, and it got worse as the market got busier.** It walked back over every price in the 15-minute window per trade: measured at 153 µs/tick at 5 trades/sec but **3653 µs/tick at 50**, i.e. O(ticks in window) PER tick. At a burst rate of several hundred trades a second it would have stopped keeping up, and the symptom would have been the lag number climbing during fast tape — the one moment the console has to be trusted. Replaced with `RollingExtremes`, two monotonic deques giving exact sliding-window high/low in O(1) amortized (188 entries held for a 60,000-tick window, zero disagreement with the scan it replaced). Cost is now flat with rate: ~10-17 µs/tick from 5/s to 500/s. |
| 2026-09-22 | **The full feature set was rebuilt on every tick to read one field.** `ZoneMonitor.on_tick` called `ZoneAccumulator.features()` per trade for the invalidation check, which needs only `adverse_excursion` — an O(1) tracked float — while `features()` runs two windowed scans (`_velocity` over 60s, `_late_delta_ratio` over 30s) plus the iceberg pass. That was ~550 µs/tick at BTC's rate. The accumulator now exposes `adverse_excursion()` on its own and the full set is built only on the scoring interval or when a signal is actually emitted. |
| 2026-09-22 | **A `maxlen` deque silently shortened a TIME window under load.** `ZoneAccumulator._recent` was `deque(maxlen=8192)` with the windowing left to time cutoffs inside the readers — fine at 45 trades/sec (89s held), but at 200/s it held 41s and at 500/s only 16s, while `_velocity` reaches back 60s and `_late_delta_ratio` 30s. So the turn detector quietly redefined "recent" during exactly the fast tape where the turn is the whole question, with no error and no visible symptom. It now evicts by time on append (`RECENT_RETENTION_MULT × late_window_ms`), holding a full 90s at every rate to 1000/s; the maxlen remains only as a backstop against broken timestamps. |
| 2026-09-22 | **A resolved level showed a verdict with no age, and is blind while it holds one.** A card read `INVALIDATED` identically whether the break happened four seconds or four minutes ago, and `ZoneMonitor` ignores that level ENTIRELY for `cooldown_ms` (300 s) — so a break-and-reclaim inside the window is missed completely. The card now shows "Ns ago, re-arms in Mm SSs" and `--cooldown` makes the window settable. The default is unchanged: shortening it trades missed reclaims for re-entering a level that is still falling. |
| 2026-09-22 | **A chart window that stretches to fit the data is wrong when the data is not the subject.** The flow chart's first version sized its price axis from min/max over the visible candles, so an hour that opened 900 above the level squashed the zone — the only thing being watched — into four pixels. The window is now anchored on the zone (zone ± 0.9× its height, extended only to keep last price visible) and candles outside are clipped, which is how Bookmap works: you set a price window and price moves through it. |
| 2026-09-22 | **A derived grid size latched before its inputs were ready.** The footprint row height was computed from the 15-minute baseline range the first time a book arrived — which is when `_prices` is still nearly empty, so the range was ~0 and the row height latched at one tick. Every footprint bar then clamped to `Math.max(1, …)` — a chart that draws nothing, and looks merely empty rather than broken. It is now derived from the narrowest active ZONE (the span actually rendered), recomputed on `sync_levels`, and only re-latched on a 2× change. General shape: anything derived from a warm-up statistic needs a readiness gate, not just a non-zero check. |
| 2026-09-22 | **`sqrt` on a sequential ramp turned a uniform order book into a solid slab.** The depth heatmap normalised resting size as `sqrt(v/max)` to handle the heavy tail, which lifted a near-uniform book to mid-ramp everywhere and buried the zone under bright blue. Linear is the honest map and reads correctly: a book IS mostly nothing with a few walls, so it should render dark almost everywhere and bright where size actually rests. |
| 2026-09-22 | **A guard clause ordered by convenience made one state unreachable.** `narrative.describe` tested `state is IDLE or features is None` before the APPROACHING branch — but there is no accumulator until price is inside a zone, so `features is None` is the normal case while approaching. Every approach printed "Waiting", the one moment the page most needed to say something else. Caught by a parametrised test over every state, not by reading the code. |
| 2026-09-22 | **A level card stops describing your trade `cooldown_ms` after it confirms.** `ZoneMonitor` holds CONFIRMED/INVALIDATED for the cooldown (default 300 s of event time), then re-arms to IDLE — and if price is still in the zone it immediately enters a NEW test with a fresh `ZoneAccumulator` and `test_count + 1`. So five minutes after an entry the card reads EVALUATING again, with the evidence that produced the signal discarded. That is correct for a level-centric engine (it is answering "what is flow doing here now") and wrong for anyone reading it as a position monitor. The signals table now prints the signal's `test_count` and greys it with `#1 -> #3` once the card has moved past it. A real position view — time since the turn, excursion vs at entry — does not exist yet. |
| 2026-09-22 | **The published invalidation price did not match where the engine actually invalidated — by a factor of two.** `_build_signal` computed the stop as `level.low - width * mult`, which for a support is `price - 2*width`, while `on_tick` killed the level at `adverse_excursion > width * mult`, i.e. `price - width`. So every signal advertised a stop twice as far as the engine's own, and `SignalLog` scored wins and losses against the wider one — inflating the logged win rate of both signals AND baselines. There is now one definition, `ZoneMonitor.invalidation_price()`, measured from the level, and a test drives price down until the engine kills it and asserts the two agree. |
| 2026-09-22 | **A phase gate calibrated against the market's range stops working when the zone shrinks.** `ZonePhase.FAILING` required `range_ratio > 1.2` — in-zone travel exceeding 1.2× the 15-minute baseline range. Once the zone became directional it is only `width` tall and price leaving it invalidates, so in-zone range can never exceed the width: a 200-wide level against a 350-wide baseline could not reach 1.2 however hard it broke. FAILING is now `adverse_excursion >= 0.6 × width` — most of the way through YOUR zone, against you, in the units you set. General lesson: a threshold in units of one thing gating a quantity bounded by another is a silent no-op waiting to happen. |
| 2026-09-22 | **An inferred tick size must be snapped — differencing two floats does not give a round number.** `_infer_tick_size` takes the smallest gap between adjacent book levels, and on the first live BTCUSDT book that came back as `0.00999999999476131` for a 0.01 tick. The bucketing error is negligible (~0.005 of a bucket at 98k), but the value is wrong, prints as broken, and would fail any equality check against the real tick. Now snapped to 8 significant digits — far more precision than any instrument's tick carries, far less than the noise, and small genuine ticks (1e-8) survive because it is significant digits rather than decimal places. |
| 2026-09-22 | **A WebSocket that accepts commands needs an Origin check — binding to loopback is not enough.** WebSockets are exempt from the same-origin policy, so any page the operator happens to have open can `new WebSocket("ws://127.0.0.1:8760/ws")` and start issuing commands. Loopback binding stops the network, not the browser. Once `live_server.py` gained `on_command` it stopped being a display and started mutating a file on disk, so the handshake now refuses an Origin that is not this server. An ABSENT Origin is allowed on purpose: browsers always send one, so no Origin means a local non-browser client, which can already edit the file directly. |
| 2026-09-22 | **A form that edits a subset of fields must INHERIT the rest, never default them.** The level editor posts price, width, side and note — it has no box for `enabled` or `expires`. Building the `Level` with `enabled=bool(raw.get("enabled", True))` meant editing a note silently re-armed a level the operator had deliberately parked, and dropped an expiry they had set. Both were silent and both would surface at the worst possible moment. `_level_from` now reads the stored level and inherits anything the payload omits. Caught by driving the real page in a browser, not by unit tests — the bug lived in the gap between the form and the constructor. |
| 2026-09-22 | **`first_seen_ms` was re-stamped on every restart.** `_parse_levels` set it to `now_ms` and `load()` only carried it forward from levels already in memory — which on a fresh process is nothing. So every monitor restart quietly reset the provenance of every level, and "this level was written before price arrived" became unfalsifiable after the first crash. It is now written to the TOML as `first_seen` and read back, with priority: in-memory value, then the file's, then now. |
| 2026-09-22 | **A read that prunes is a read that destroys.** `LevelMemory.counts()` originally called `record.prune()` before counting, so every diagnostic call, dashboard poll and snapshot silently deleted the history it was reporting on — and the deletion was invisible because the number returned was still correct *that* time. `counts()` now filters by age without mutating; pruning lives only in `record()` and `load()`. The same read also must be fed an **event** timestamp, never wall clock: under replay or backfill the two diverge by days and a wall-clock age filter would drop everything. |
| 2026-09-22 | **A level in cooldown cannot record an outcome, which makes cooldown-length test sequences lie.** After INVALIDATED or CONFIRMED a `ZoneMonitor` ignores ticks entirely for `cooldown_ms` (default 300 s of *event* time). A test that breaks a level then re-enters it 5 seconds later never re-arms, so the second outcome is never recorded — which looks exactly like a broken outcome-detection path. Advance event time past the cooldown between outcomes. Related: a confirmation that later breaks is caught **during** the cooldown by an explicit invalidation-price check, because by the time the cooldown expires price may have wandered back and the exit test would score it a hold. |
| 2026-09-22 | **The iceberg ratio must divide by MAX displayed, not last or average.** `iceberg_ratio = traded_at_price / max_ever_displayed_there`. A genuine 200-lot order eaten 200→150→…→0 has max-displayed 200 against 200 traded — ratio 1.0, correctly not an iceberg. An order that only ever shows 5 while 200 trades through it has ratio 40 — correctly an iceberg. Using last-seen or average displayed inverts this: the real order would look hidden as its display drained toward zero. Pinned by `test_iceberg.py::TestDiscriminator` in both directions. Needs BOTH halves (book + tape), so it only fires with `--depth`; without a book the ratio stays 0, which is an absent signal rather than a false one. |
| 2026-09-22 | **Infer tick size from book level spacing, not from trade prices.** Book levels sit exactly on tick boundaries, so one snapshot gives an exact answer where trades would need statistics. `FlowEngine._infer_tick_size` latches it once — re-deriving per snapshot would let a single odd book shrink it, and the tick does not change intraday. Price bucketing for the iceberg ratio depends on it entirely. |
