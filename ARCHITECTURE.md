...
| IC Markets Feed | `src/data/feeds/icmarkets_feed.py` | `ctrader-open-api`, Twisted (threaded reactor), Types | TradingEngine (gold) | ✅ Session 23 D1 — full 5-stage auth walk (AppAuth → GetAccountList → AccountAuth → SymbolsList → SubscribeSpots + LiveTrendbar), `_send_quiet()` wraps client.send with Deferred errback to absorb 5s subscribe-ack timeouts, SpotEvent bid/ask → Tick + embedded ProtoOATrendbar → Candle dispatch via `asyncio.run_coroutine_threadsafe`. Reactor runs in worker thread, asyncio stays on the main loop. Live validated on XAUUSD 1h. |
