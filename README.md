...
**What's built on `feat/gold-refactor` (gold stack, pre-merge):** All of the above plus a separate leveraged backtest engine for XAUUSD with CFD-accurate margin accounting, a two-book architecture (institutional Tiers 1-4 + aggressive Tier 5), Brownian-bridge intrabar path reconstruction, IC Markets cost model (0.13 pip spread + $3/lot/side commission), `donchian_gold` (tuned to +38% / 12% DD / Calmar 1.675 / Sharpe 1.275 on 2yr XAUUSD 1h), M3S `request_leverage` policy API with 5 reason codes + geometric-mean blend in `risk_scalar` for leverage > 1×, inline leverage gates for in-process fast-path, RCU versioned portfolio view, SQLite `leverage_grants` store, and 920 tests passing. Three Tier 5 aggressive strategies (`candle_burst_hunter`, `news_spike_fade`, `hedged_structure_play`) are **infrastructure-complete but NOT alpha-ready** — they have correct state machines but the entry triggers over-fire on bar-level data and need dedicated alpha research before paper trading.
