...
| **Tier 5 alpha research** | Task #80 — dedicated research pass | ❌ **ALL 3 KILLED** | `1560e56` | — | `news_spike_fade`: premise confirmed (95.7% retracement rate on >=30 pip spikes) but bar-level execution CANNOT capture tick-level fade timing. `hedged_structure_play`: design mismatched to gold's sustained uptrend, 386 trades / 2yr is over-trading, all configs wipe. `candle_burst_hunter`: requires tick-level mid-bar velocity detection, cannot work on bar data. Obituaries: #92, #93, #94. With G.5b's M1 path model now available, tick-level mid-bar velocity becomes feasible via sub-bar counting — revisit these with `sub_bar_count=5` if the aggressive sub-book is reopened. |
...
**Test counts:** 982 passing on `feat/gold-refactor` as of G.5b (734 baseline + G.0/G.0c/G.1/G.2a-G.2f/G.2g/G.2h + G.5/G.5b additions).
