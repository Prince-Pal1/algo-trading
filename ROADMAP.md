...
| Phase 4 skeleton | IC Markets cTrader feed + executor + OAuth helper (pip install ctrader-open-api) | ✅ COMPLETE | `<pending>` | 15 | `icmarkets_feed.py` + `icmarkets_executor.py` + `ctrader_token_helper.py` + `.env.example` all built. Unit tests with mocks green. App registered at openapi.ctrader.com; credentials in hand; sandbox read-only token available now; `trading` scope gated on Spotware KYC (up to 3 business days). |
