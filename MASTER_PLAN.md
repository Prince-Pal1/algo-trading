...
| Strategy storage layout (2026-04-15, task #117/#127) | `config/catalog.toml` is **research-only**; runtime versioned registry lives in SQLite tables `strategies` + `strategy_versions` in `data/trades.db` | Two distinct concerns: (a) catalog.toml = paper citations, edge summaries, effort estimates curated by humans; (b) SQLite tables = runtime variants auto-captured from `run_deep_backtest()`. Mixing the two would force every backtest run to rewrite the TOML file (git-noise hell) AND require sync logic that violates Rule 1's "single source of truth per fact" rule. The two layers do NOT sync — if you want to surface catalog metadata in the dashboard, read both stores. See `docs/LEVERAGE_STRATEGY_DESIGN.md` and `src/strategies/storage.py` module docstring. |
| Deep_backtest does NOT write to `backtest_runs` (2026-04-15, task #117/#126) | Deep backtest writes filesystem reports + `strategy_versions` rows; `backtest_runs` (used by `result_store.save_run()`) stays unchanged | The two layers are intentionally orthogonal: `backtest_runs` is per-individual-run with full equity curves and trade ledgers (used by Streamlit "Backtest Runs" page); deep_backtest is a matrix sweep where each cell would be its own row, which would explode `backtest_runs` with hundreds of rows per matrix run. Stage 3 task #124 adds an OPTIONAL `backtest_run_id` FK on `strategy_versions` so a Streamlit deep-link can connect a version to a single equity curve when the user wants drill-down — but the FK is null by default and deep_backtest never populates it. Touching `backtest_runs` from deep_backtest was rejected for blast-radius reasons (`result_store.save_run` is used by 4+ existing call sites). |
